...
def _apply_prob_ceiling(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Called once when predictions load — result cached in session state.
    No Python loops, all numpy operations.
    """
    ...
    # Pull columns as numpy arrays — fillna keeps things safe
    ...
    df["confidence"]  = pd.cut(final,
        bins  = [-np.inf, 0.14, 0.22, 0.32, np.inf],
        labels= ["Low", "Medium", "High", "Elite"]).astype(str)
    df["conf_goals"]  = df["confidence"]
    if "projected_sog" in df.columns:
        sog = df["projected_sog"].fillna(0).values.astype(float)
        df["conf_sog"] = pd.cut(sog,
...
def render_nhl(selected_date_str, force_retrain):
    ...
    # Recalculate per-category confidence if not in parquet
