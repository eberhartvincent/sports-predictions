...
def _nhl_fetch_results(date_str: str) -> dict:
    """
    Fetch NHL game results for a given date.
    Returns {player_id: {goals, shots}} dict.
    Tries multiple endpoints in order.
    """
    # Primary: score endpoint (public, no auth)
    for url in [
        ...
        try:
            ...
            for game in games:
                ...
                if not gid: continue
                # Try to get boxscore
                bs_url = f"https://api-web.nhle.com/v1/gamecenter/{gid}/boxscore"
                bs_r   = requests.get(bs_url, headers=HEADERS, timeout=15)
                if bs_r.status_code != 200: continue
                bs = bs_r.json()
