...
def render_mlb(selected_date: str, force_retrain: bool):
    ...
    if st.session_state.mlb_preds.empty or (_selected != _session_date) or (_is_today and _disk_mtime and _disk_mtime != _session_mtime):
        ...
        if _hist and _hist.exists():
            ...
            _hist_preds = _pd.read_parquet(_hist)
            stored = load_predictions("mlb")   # get structure
            stored["predictions"] = _hist_preds
        else:
            stored = load_predictions("mlb")
        _stored_preds = stored.get("predictions", pd.DataFrame())
        if isinstance(_stored_preds, dict): _stored_preds = pd.DataFrame()
        if not _stored_preds.empty:
            stored["predictions"] = _stored_preds
        if not stored.get("predictions", pd.DataFrame()).empty:
